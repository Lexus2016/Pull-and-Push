...
Нужны **Python 3.10+** и **Git**. Этого достаточно, чтобы открыть панель и создать проект;
чтобы реально запустить оптимизацию, дополнительно нужен один CLI ИИ-агента (шаг 5).
...
**2 — Создать изолированное окружение и установить**
...
Открой напечатанный URL — по умолчанию **http://127.0.0.1:8765**.

**4 — Создать первый проект (в браузере)**

Нажми **🚀 Новый проект из примера** → выбери карточку → дай название → опиши цель →
**Создать** → нажми **▶ Запуск**.

**5 — Чтобы запустить реальную оптимизацию, добавь CLI агента**

Pull-and-Push управляет уже установленным CLI для ИИ-кодинга; никаких API-ключей здесь не
хранится. Установи и войди хотя бы в один, затем выбери его в проекте: **claude**
(Anthropic), **codex** (OpenAI) или **opencode**. Совет: бери *разных* провайдеров для
исполнителя и валидатора — одинаковая модель с обеих сторон имеет общие слепые зоны.
